...
static const uint32_t CONFIG_CHANGE_PAUSE_MS = 0;
...
void MotorTask::run() {
    ...
    uint32_t last_config_change_ = 0; // Added to track the last configuration change time

    while (1) {
        motor.loopFOC();
        
        // Skip all motor processing during cooldown period after config changes
        if (millis() - last_config_change_ < CONFIG_CHANGE_PAUSE_MS) {
            // Set motor torque to zero during pause
            motor.move(0);
            delay(1);
            continue;
        }
        ...
        if (xQueueReceive(queue_, &command, 0) == pdTRUE) {
            switch (command.command_type) {
                ...
                case CommandType::CONFIG: {
                    ...
                    last_config_change_ = millis(); // Record the time of the configuration change
                    break;
                }
